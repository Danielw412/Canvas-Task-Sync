  ...
  return <div className="run-detail-layout">
    <section className="run-workspace">
      ...
      <section className="plan-section">
        ...
        {filtered.length ? <div className="table-frame plan-table"><div className="data-table data-table--plan data-table__header"><span>Action</span><span>Task</span><span>Due</span><span>Destination</span><span /></div>{filtered.map((action, index) => <button className="data-table data-table--plan data-row" key={`${action.logical_id ?? action.title}-${index}`} onClick={() => setSelectedAction(action)}><ActionMark kind={action.kind} /><strong>{action.title}</strong><span>{action.due_uncertain ? 'Due date uncertain' : action.due_date ? new Intl.DateTimeFormat(undefined, { weekday: 'short', month: 'short', day: 'numeric' }).format(new Date(`${action.due_date}T12:00:00`)) : 'No due date'}</span><span>{action.task_list ?? '-'}</span><ChevronRight size={16} /></button>)}<div className="table-hint"><Clock3 size={16} /> Select a task to view its decision and evidence.</div></div> : <EmptyState title={isActive ? 'Building the review plan' : 'No items in this view'} body={isActive ? 'Actions will appear when source comparison completes.' : 'Choose another plan filter.'} />}
      </section>
    ...
    <aside className="run-inspector inspector-rail">
      <section className="rail-section run-summary"><h2>Run details</h2><div className="summary-grid"><span>Run ID</span><strong>{run.id}</strong><span>Trigger</span><strong>{run.trigger === 'schedule' ? 'Schedule' : 'Manual'}</strong><span>Extraction</span><strong>{run.extraction_mode ? humanize(run.extraction_mode) : '-'}</strong><span>Cache</span><strong>{run.events.some((event) => event.metadata.cache === 'hit') ? 'Hit' : 'Miss'}</strong><span>Remote tasks</span><strong>{String(run.events.find((event) => event.stage === 'compare_google_tasks')?.metadata.remote_task_count ?? '-')}</strong></div><div className="current-step"><span>Current step</span><p>{isActive ? <LoaderCircle className="spin" size={17} /> : <CheckCircle2 className="tone-success" size={17} />}{latestEvent?.message ?? 'Waiting for the first event.'}</p></div></section>
