...
export default function RunDetailPage() {
  ...
  const changeCount = actions.filter((action) => ['create', 'update'].includes(action.kind)).length
  const attentionItems = actions.filter((action) => attention(action.kind))
  const filtered = useMemo(() => actions.filter((action) => {
    if (planFilter === 'changes') return ['create', 'update'].includes(action.kind)
  ...
  return <div className="run-detail-layout">
    <section className="run-workspace">
      <header className="run-heading">
        <div><div className="breadcrumbs"><Link to="/runs">Runs</Link><span>/</span><span>Run #{run.id}</span></div><h1>{run.requested_mode === 'health' ? `Checking ${run.course_name ?? run.course_id}` : run.status === 'applying' ? `Applying ${run.course_name ?? run.course_id}` : `Previewing ${run.course_name ?? run.course_id}`}</h1><p>Started {formatDateTime(run.started_at ?? run.created_at)} · {run.trigger === 'schedule' ? 'Scheduled trigger' : 'Manual trigger'}</p></div>
      ...
      <section className="plan-section">
        ...
        {filtered.length ? <div className="table-frame plan-table"><div className="data-table data-table--plan data-table__header"><span>Action</span><span>Task</span><span>Due</span><span>Source</span><span>Reason</span><span /></div>{filtered.map((action, index) => <button className="data-table data-table--plan data-row" key={`${action.logical_id ?? action.title}-${index}`} onClick={() => setSelectedAction(action)}><ActionMark kind={action.kind} /><strong>{action.title}</strong><span>{action.due_date ? new Intl.DateTimeFormat(undefined, { weekday: 'short', month: 'short', day: 'numeric' }).format(new Date(`${action.due_date}T12:00:00`)) : 'No due date'}</span><span>Google Slides</span><span>{action.reason}</span><ChevronRight size={16} /></button>)}<div className="table-hint"><Clock3 size={16} /> Click a row to view details, evidence, and field-level comparisons.</div></div> : <EmptyState title={isActive ? 'Building the review plan' : 'No items in this view'} body={isActive ? 'Actions will appear when source comparison completes.' : 'Choose another plan filter.'} />}
      </section>

      <section className="apply-band panel"><div><ShieldCheck size={23} /><p><strong>Only Create and Update actions will write to Google Tasks.</strong><span>Nothing is ever deleted automatically.</span></p></div><div><Button variant="secondary" onClick={() => navigate('/runs')}>Discard preview</Button><Button icon={LockKeyhole} disabled={run.status !== 'awaiting_approval' || changeCount === 0 || Boolean(run.test_rebase_week)} onClick={() => setConfirmApply(true)}>Apply {changeCount} {changeCount === 1 ? 'change' : 'changes'}</Button>{run.status !== 'awaiting_approval' ? <small>Available when preview completes</small> : null}</div></section>
    </section>
...
function ActionDrawer({ action, onClose }: { action: SyncAction; onClose: () => void }) {
  return <div className="drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose() }}><aside className="detail-drawer" role="dialog" aria-modal="true" aria-label="Plan item details"><header><div><ActionMark kind={action.kind} /><h2>{action.title}</h2></div><button className="icon-button" onClick={onClose} aria-label="Close details"><X size={20} /></button></header><div className="detail-drawer__body"><section><h3>Decision</h3><p>{action.reason}</p></section><section><h3>Exact source evidence</h3><blockquote>{action.evidence || action.desired?.source_text || 'No source evidence was retained for this item.'}</blockquote></section><section><h3>Desired task</h3><dl><dt>Due date</dt><dd>{action.due_date ?? 'None'}</dd><dt>Due basis</dt><dd>{action.desired?.due_basis ?? '—'}</dd><dt>Classification</dt><dd>{action.desired?.classification ?? '—'}</dd><dt>Action kind</dt><dd>{action.desired?.action_kind ?? '—'}</dd><dt>Remote task ID</dt><dd>{action.remote_task_id ?? 'Not assigned'}</dd></dl></section>{action.desired?.source_url ? <a className="inline-link" href={action.desired.source_url} target="_blank" rel="noreferrer">Open source page <ArrowLeft className="rotate-180" size={16} /></a> : null}</div></aside></div>
}
