...
import { agendaWeekOptions, formatDateTime, formatDuration, mutateJson, useOverview } from '../lib/api'
    ...
    <section className="overview-main">
      <header className="page-heading overview-heading">
        ...
        <div className="overview-actions">
          ...
          <label className="select-control"><span className="sr-only">Agenda source</span><select aria-label="Agenda source" value={selectedAcquisitionStrategy} onChange={(event) => setAcquisitionStrategy(event.target.value as AcquisitionStrategy)}><option value="auto">{course?.settings.source.type === 'none' ? 'Canvas API · no fallback' : 'Canvas first · automatic fallback'}</option><option value="canvas_api" disabled={!course?.settings.canvas_course_id}>Canvas API only</option>{course?.settings.source.type !== 'none' ? <option value="configured_source">{course?.settings.source.type === 'browser' ? 'Chrome capture only' : 'Configured API source only'}</option> : null}</select></label>
      ...
      <section className="source-workspace panel">
        <div className="source-summary">
          <div className="source-summary__item"><span className="source-icon source-icon--slides"><Presentation size={19} /></span><div><span>Source</span><strong>{course?.settings.canvas_course_id ? `Canvas API · Course ${course.settings.canvas_course_id}` : course?.settings.source.type === 'browser' ? 'Chrome capture · Configured fallback' : 'Google Slides API · Configured source'}</strong></div></div>
          <div className="source-summary__item"><span className="source-icon source-icon--tasks"><Target size={19} /></span><div><span>Task destinations</span><strong>Assignments · {course?.settings.task_list ?? 'Not configured'} · Tests · {course?.settings.assessment_task_list ?? 'Not configured'}</strong></div></div>
